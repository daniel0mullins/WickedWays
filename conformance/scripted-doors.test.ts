...
const start = readFileSync(join(here, "fixtures/scripted-doors.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/scripted-doors.catalog.json"), "utf8");
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/scripted-doors.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
...
describe("scripted-doors differential conformance", () => {
  it("Rust scripted exits match the TS doorBehavior oracle per step (cues + snapshot + view)", () => {
