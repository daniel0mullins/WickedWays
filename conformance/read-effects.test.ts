...
const start = readFileSync(join(here, "fixtures/read-effects.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/read-effects.catalog.json"), "utf8");
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/read-effects.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
...
describe("read-effects differential conformance", () => {
  it("Rust scripted onRead drains Sanity before the lore cue, matching the TS oracle", () => {
