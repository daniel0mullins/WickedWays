...
const start = readFileSync(join(here, "fixtures/spawn.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/spawn.catalog.json"), "utf8");
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/spawn.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
...
describe("spawn differential conformance", () => {
