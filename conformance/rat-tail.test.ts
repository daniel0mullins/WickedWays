...
const start = readFileSync(join(here, "fixtures/rat-tail.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/rat-tail.catalog.json"), "utf8");
const golden = JSON.parse(readFileSync(join(here, "fixtures/rat-tail.golden.json"), "utf8")) as {
  seed: number;
  commands: unknown[];
  steps: Array<{ command: unknown; cues: unknown; snapshot: unknown; view: unknown }>;
};

describe("rat-tail onUse differential conformance", () => {
  it("Rust scripted onUse (+1 Sanity, consumed) matches the TS ratTail oracle per step", () => {
