...
const start = readFileSync(join(here, "fixtures/scripted-victory-partydown.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/scripted-victory-partydown.catalog.json"), "utf8");
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/scripted-victory-partydown.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
...
describe("scripted-victory-partydown differential conformance", () => {
  it("Rust scripted victory matches the TS closure oracle per step (cues + snapshot + view)", () => {
