...
const start = readFileSync(join(here, "fixtures/keyed-exit.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/keyed-exit.catalog.json"), "utf8");
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/keyed-exit.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
...
describe("keyed-exit differential conformance", () => {
