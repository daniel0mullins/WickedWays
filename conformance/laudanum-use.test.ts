...
const start = readFileSync(join(here, "fixtures/laudanum-use.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/laudanum-use.catalog.json"), "utf8");
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/laudanum-use.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
...
describe("laudanum-use differential conformance", () => {
  it("Rust scripted onUse matches the real HH laudanum closure per step (+6 Sanity, uncapped up)", () => {
