...
const start = readFileSync(join(here, "fixtures/sees-in-dark.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/sees-in-dark.catalog.json"), "utf8");
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/sees-in-dark.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
...
describe("sees-in-dark differential conformance", () => {
