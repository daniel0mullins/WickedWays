...
const start = readFileSync(join(here, "fixtures/mechanics-action.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/mechanics-action.catalog.json"), "utf8");
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/mechanics-action.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
...
describe("mechanics-action differential conformance", () => {
