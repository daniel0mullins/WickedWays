...
const start = readFileSync(join(here, "fixtures/mechanics-turnend.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/mechanics-turnend.catalog.json"), "utf8");
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/mechanics-turnend.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
...
describe("mechanics turn-end differential conformance", () => {
  it("Rust replay matches the TS oracle per step (cues + snapshot + view)", () => {
    // Guard the fixture shape itself: the golden must be the fixed-seed,
    // two-step (startTurn, attack) at-cap-mob stream — a degenerate
    // regeneration must not pass silently.
    expect(golden.seed).toBe(1);
    expect(golden.steps.length).toBe(2);
