...
const start = readFileSync(join(here, "fixtures/mob-drop.start.snapshot.json"), "utf8");
const catalogJson = readFileSync(join(here, "fixtures/mob-drop.catalog.json"), "utf8");
const golden = JSON.parse(readFileSync(join(here, "fixtures/mob-drop.golden.json"), "utf8")) as {
  seed: number; commands: unknown[];
  steps: Array<{ command: unknown; cues: unknown; snapshot: unknown; view: unknown }>;
};

describe("mob-drop differential conformance", () => {
