...
const start = readFileSync(
  join(here, "fixtures/afflictions.start.snapshot.json"),
  "utf8",
);
const catalogJson = readFileSync(
  join(here, "fixtures/afflictions.catalog.json"),
  "utf8",
);
const golden = JSON.parse(
  readFileSync(join(here, "fixtures/afflictions.golden.json"), "utf8"),
) as {
  seed: number;
  commands: unknown[];
  steps: Array<{
...
describe("afflictions differential conformance", () => {
