...
mod tests {
    ...
    fn attack_entry_roundtrips() {
        let e = ActionHistoryEntry::Attack {
            round: 1,
            target: TargetRef { id: CharacterId("c1".to_string()), name: "Goblin".to_string() },
        };
        let s = serde_json::to_string(&e).unwrap();
    ...
    fn pick_up_entry_roundtrips() {
        let e = ActionHistoryEntry::PickUp {
            round: 2,
            items: alloc::vec![
                ItemRef { id: ItemId("i1".to_string()), name: "Sword".to_string() },
            ],
        };
        let s = serde_json::to_string(&e).unwrap();
        let back: ActionHistoryEntry = serde_json::from_str(&s).unwrap();
    ...
    fn drop_entry_roundtrips() {
        let e = ActionHistoryEntry::Drop {
            round: 2,
            items: alloc::vec![
                ItemRef { id: ItemId("i2".to_string()), name: "Shield".to_string() },
            ],
        };
        let v = serde_json::to_value(&e).unwrap();
    ...
    fn escape_entry_roundtrips() {
        let e = ActionHistoryEntry::Escape { round: 4, success: true };
        let s = serde_json::to_string(&e).unwrap();
        let back: ActionHistoryEntry = serde_json::from_str(&s).unwrap();
        assert_eq!(back, e);
        let v = serde_json::to_value(&e).unwrap();
    ...
    fn fumble_entry_roundtrips() {
        let e = ActionHistoryEntry::Fumble { round: 5, action: "attack".to_string() };
        let v = serde_json::to_value(&e).unwrap();
