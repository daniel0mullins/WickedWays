...
mod tests {
    ...
    fn calls_map_to_typed_nodes() {
        assert_eq!(
            p("hasKey(actor, 'vault')"),
            json!({"kind":"hasKey","of":{"kind":"actor"},"keyCode":"vault"})
        );
        assert_eq!(
            p("hasItem(party[0], 'journal')"),
    ...
    fn call_arity_error_is_expr_parse() {
        ...
        assert!(matches!(
            parse_expr("hasKey(actor)", Span { line: 1, col: 1 }).unwrap_err(),
    ...
    fn non_string_second_arg_is_expr_parse() {
        ...
        assert!(matches!(
            parse_expr("hasKey(actor, round)", Span { line: 1, col: 1 }).unwrap_err(),
            CompileError::ExprParse { .. }
        ));
    }
    ...
    fn syntax_error_is_expr_parse() {
        assert!(matches!(
            parse_expr("round ==", Span { line: 1, col: 1 }).unwrap_err(),
            CompileError::ExprParse { .. }
        ));
    }
    ...
    fn action_and_element_subjects() {
        assert_eq!(
            p("action.kind"),
            json!({"kind":"get","field":"kind","of":{"kind":"action"}})
        );
        assert_eq!(
            p("element.status"),
    ...
    fn map_lit_and_has_and_lookup() {
        ...
        assert_eq!(
            p("mapLit('Parlor', 'A mildewed parlor.')"),
            json!({"kind":"mapLit","entries":{"Parlor":"A mildewed parlor."}})
        );
        assert_eq!(
            p("has(mapLit('Parlor', 'x'), action.room.name)"),
            json!({
            "kind":"has",
        ...
        assert_eq!(
            p("lookup(mapLit('Parlor', 'x'), action.room.name)"),
            json!({
            "kind":"lookup",
            "map":{"kind":"mapLit","entries":{"Parlor":"x"}},
            "key":{"kind":"get","field":"name","of":{"kind":"get","field":"room","of":{"kind":"action"}}}})
    ...
    fn state_get_in_expression() {
        assert_eq!(
            p("stateGetIn('seen', action.room.name, false)"),
            json!({
    ...
    fn str_length_first_concat() {
        ...
        assert_eq!(
            p("length(party)"),
            json!({"kind":"length","list":{"kind":"party"}})
        );
        // `first(party)` is the First node, distinct from the subscript `party[0]`→Index.
        assert_eq!(
            p("first(party)"),
            json!({"kind":"first","list":{"kind":"party"}})
        );
        assert_eq!(
            p("concat(str(round), '/', str(maxRounds))"),
            json!({
            "kind":"concat","parts":[
    ...
    fn some_every_includes_quantifiers() {
        ...
        assert_eq!(
            p("every(party, includes(element.status, 'ko'))"),
            json!({
    ...
    fn deeply_nested_input_errors_not_panics() {
        ...
        assert!(matches!(
            parse_expr(&parens, Span { line: 1, col: 1 }).unwrap_err(),
            CompileError::ExprParse { .. }
        ));
        let bangs = format!("{}actor", "!".repeat(5000));
        assert!(matches!(
            parse_expr(&bangs, Span { line: 1, col: 1 }).unwrap_err(),
            CompileError::ExprParse { .. }
        ));
        let ternary = format!(
            "{}actor{}",
        ...
        assert!(matches!(
            parse_expr(&ternary, Span { line: 1, col: 1 }).unwrap_err(),
            CompileError::ExprParse { .. }
        ));
        ...
        assert!(matches!(
            parse_expr(&gets, Span { line: 1, col: 1 }).unwrap_err(),
            CompileError::ExprParse { .. }
        ));
        let indexes = format!("party{}", "[0]".repeat(5000));
        assert!(matches!(
            parse_expr(&indexes, Span { line: 1, col: 1 }).unwrap_err(),
            CompileError::ExprParse { .. }
        ));
        let bins = format!("round{}", " + 1".repeat(5000));
        assert!(matches!(
            parse_expr(&bins, Span { line: 1, col: 1 }).unwrap_err(),
            CompileError::ExprParse { .. }
        ));
    }
    ...
    fn double_quoted_string_with_apostrophes() {
        ...
        assert_eq!(
            p("\"A page clears: 'They would not.'\""),
