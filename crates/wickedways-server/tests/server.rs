...
use wickedways_server::store::{
    CampaignRecord, CampaignStore, MembershipState, SqliteStore, StoreError,
};
...
fn denied_with(log: &Arc<Mutex<Vec<ServerMsg>>>, needle: &str) -> bool {
    has(
        log,
        |m| matches!(m, ServerMsg::Denied { reason } if reason.contains(needle)),
    )
}
...
async fn submit_before_any_join_is_not_authenticated() {
    let s = server(None);
    let (mut c, log) = conn(&s, 1);
    c.handle(submit("demo", json!({ "kind": "nextPlayer" })))
        .await;
...
async fn a_commit_acks_the_sender_and_broadcasts_entry_to_others() {
    ...
    gm.handle(submit("demo", json!({ "kind": "nextPlayer" })))
        .await;
    assert!(committed(&gm_log), "submitter gets committed");
    assert!(
        has(&ada_log, |m| matches!(m, ServerMsg::Entry { .. })),
        "other participant gets entry"
    );
}
...
async fn a_non_gm_submitting_a_gm_command_is_denied() {
    ...
    ada.handle(submit("demo", json!({ "kind": "nextPlayer" })))
        .await;
...
async fn gm_assign_lets_the_assigned_identity_act_and_unassign_revokes() {
    ...
    gm.handle(ClientMsg::UnassignSeat {
        campaign_id: "demo".into(),
        character_id: ADA.into(),
    })
    .await;
...
async fn transfer_gm_moves_the_gm_role() {
    ...
    gm.handle(join("demo", "gm", 0)).await;
    gm.handle(ClientMsg::TransferGm {
        campaign_id: "demo".into(),
        identity: "ada".into(),
    })
    .await;
    ...
    old.handle(submit("demo", json!({ "kind": "nextPlayer" })))
        .await;
    assert!(
        denied_with(&old_log, "not authorized for this seat"),
        "the old GM is no longer GM"
    );
}
...
async fn presence_is_broadcast_on_join() {
    ...
    assert!(has(
        &log,
        |m| matches!(m, ServerMsg::Presence { gm, .. } if gm.identity == "gm" && gm.online)
    ));
}
...
async fn an_identity_stays_online_while_any_connection_is_live() {
    ...
    assert!(
        has(
            &log,
            |m| matches!(m, ServerMsg::Presence { gm, .. } if gm.online)
        ),
...
async fn get_snapshot_returns_a_snapshot_and_null_for_unknown() {
    let s = server(None);
    let (mut c, log) = conn(&s, 1);
    c.handle(ClientMsg::GetSnapshot {
        campaign_id: "demo".into(),
    })
    .await;
    assert!(has(
        &log,
        |m| matches!(m, ServerMsg::Snapshot { snapshot, .. } if !snapshot.is_null())
    ));

    c.handle(ClientMsg::GetSnapshot {
        campaign_id: "nope".into(),
    })
    .await;
    assert!(has(
        &log,
        |m| matches!(m, ServerMsg::Snapshot { seq: 0, snapshot } if snapshot.is_null())
    ));
}
...
async fn persists_and_resumes_a_campaign_and_its_seats_across_a_restart() {
    ...
    assert!(
        committed(&p2_log),
        "the resumed seat lets p2 act as Ada after a restart"
    );
}
...
async fn a_store_load_failure_denies_the_join() {
    ...
    c.handle(join("demo", "ada", 0)).await;
    assert!(
        denied_with(&log, "unknown campaign"),
        "a load failure reads as not-found, not a hang"
    );
}
...
async fn fails_closed_when_a_persisted_snapshot_schema_does_not_match() {
    ...
    assert!(
        denied_with(&log, "unknown campaign"),
        "a schema mismatch refuses to resume (fail-closed)"
    );
}
